...
- 了解 LangSmith、Langfuse 等 LLM 可观测性工具解决的问题及其基本接入位置。
...
10. LLM 应用可观测性，以及 LangSmith、Langfuse 简介；
11. 常见误区、实践建议与总结。
...
本节保持入门深度，先说明传统应用日志难以完整回答一次 Agent 执行经过了哪些节点、发起了哪些模型与工具调用、消耗了多少 Token、为何走入某个分支等问题，再介绍 Trace、Span、运行树、数据集、评测和反馈等基础概念。

工具部分简要覆盖：

- LangSmith：与 LangChain/LangGraph 生态结合紧密，适合追踪链与图执行、调试、数据集和评测；
- Langfuse：开源、可自托管的 LLM 工程平台，适合追踪、Prompt 管理、成本与质量分析及评测；
- 两者的共同能力、主要差异、部署取舍，以及避免重复采集的选型原则。

结合本项目说明可观测性接入点：模型初始化与调用、LangGraph 主图和子图执行、Tool 调用、`thread_id`、项目 ID 与事务 ID 的关联，以及 SSE 输出前后的错误与耗时。本文只给出接入思路和最小示例，不实施监控集成。
...
- 涉及当前 API、产品定位以及 Dify、LangSmith、Langfuse 能力的内容以各自官方资料为准；
...
- 同时覆盖 LangChain/LangGraph 关系、核心概念、标准开发流程、本项目代码用法、Dify 对比和 LLM 可观测性；
- 简要介绍 LangSmith 与 Langfuse，并说明本项目可接入的关键位置；
- 至少包含分层关系图和项目请求生命周期图；
- 每个主要项目案例均提供准确的文件路径；
