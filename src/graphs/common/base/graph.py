...
from src.graphs.common.base.nodes import AnyOptimizationDocNodes
from src.graphs.common.base.routes import AnyOptimizationDocRoutes
...
def create_group_member_review_optimization_doc_agent(
        state_schema: type[AnyGroupMemberReviewOptimizationDocState],
        review_optimization_doc_node: Callable,
        review_optimization_doc_tool_router: Callable,
        tool_list: list[BaseTool] = with_common_tools,
        messages_key: str = "private_messages"
) -> CompiledStateGraph:
    """创建评审子图（供多角色并发评审使用）"""
    agent_builder = StateGraph(state_schema)

    agent_builder.add_node("review_optimization_doc_node", review_optimization_doc_node)
    agent_builder.add_node("review_optimization_doc_tool_node", ToolNode(tool_list, messages_key=messages_key))

    agent_builder.add_edge(START, "review_optimization_doc_node")
    agent_builder.add_conditional_edges(
        "review_optimization_doc_node",
        review_optimization_doc_tool_router,
        ["review_optimization_doc_tool_node", END]
    )
    agent_builder.add_edge("review_optimization_doc_tool_node", "review_optimization_doc_node")

    agent = agent_builder.compile()
    return agent


def create_optimization_doc_agent(
        state_schema: type[AnyOptimizationDocState],
        group_member_review_state_schema: type[AnyGroupMemberReviewOptimizationDocState],
        nodes: AnyOptimizationDocNodes,
        routes: AnyOptimizationDocRoutes,
    ...
    agent_builder.add_node("group_member_review_optimization_doc_node",
                           create_group_member_review_optimization_doc_agent(
                               group_member_review_state_schema,
