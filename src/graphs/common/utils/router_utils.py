...
def tool_router(state: AnyState, tool_node: str, next_node: str, messages_key="private_messages") -> str:
    destination_node = next_node
    if (isinstance(state[messages_key], list)
            and state[messages_key]
            and isinstance(state[messages_key][-1], AIMessage)
            and state[messages_key][-1].tool_calls):
        destination_node = tool_node
    return destination_node


def rollback_tool_router(state: AnyState, self_node: str, tool_node: str, next_node: str,
                         messages_key="private_messages") -> str:
    destination_node = next_node
    if (isinstance(state[messages_key], list)
            and state[messages_key]
            and isinstance(state[messages_key][-1], AIMessage)
...
def optimize_doc_tool_router(state: AnyState,
                             ...
                             messages_key="private_messages") -> str | list[Send]:
    if (isinstance(state[messages_key], list)
            and state[messages_key]
            and isinstance(state[messages_key][-1], AIMessage)
            and state[messages_key][-1].tool_calls):
