...
def distinct_reducer(current: list, update: list | ReducerActionType):
    ...
    match update:
        case ReducerActionType.RESET:
            logger.info(
                f"trans_id:{trans_id_ctx.get()} 合并:{gutils.get_func_name()} 元素类型:{item_type} 总记录:{len(current)} 清空记录")
            return []
        case _:
            ...
            logger.info(
                f"trans_id:{trans_id_ctx.get()} 合并:{gutils.get_func_name()} 元素类型:{item_type} 总记录:{len(current)} 新记录:{len(update)}")
            return current
