...
def create_group_member_review_agent() -> CompiledStateGraph:
    ...
    agent_builder.add_conditional_edges(
        "review_system_api_node",
        routes.review_system_api_tool_router,
        ["review_system_api_tool_node", END]
    )
    agent_builder.add_edge("review_system_api_tool_node", "review_system_api_node")
...
def create_agent() -> CompiledStateGraph:
    ...
    工作流程：生成方案 → 审核方案 → 优化 → 评审（多角色并发）→ 聚合
    - 生成方案节点：生成优化系统接口的方案
    - 审核方案节点：审核优化方案，根据结果决定后续流程
    - 优化节点：根据上下文优化系统接口列表
    - Review 子图：团队成员分别评审
    ...
    # 聚合节点
    agent_builder.add_node("review_system_api_aggregator_node", nodes.review_system_api_aggregator_node)

    agent_builder.add_edge(START, "generate_optimization_system_api_plan_node")

    agent_builder.add_conditional_edges(
        "generate_optimization_system_api_plan_node",
        routes.generate_optimization_system_api_plan_tool_router,
        ["generate_optimization_system_api_plan_tool_node", "review_optimization_system_api_plan_node"]
    )
    agent_builder.add_edge("generate_optimization_system_api_plan_tool_node",
                           "generate_optimization_system_api_plan_node")
    ...
    agent_builder.add_edge("review_optimization_system_api_plan_tool_node", "review_optimization_system_api_plan_node")
    ...
    agent_builder.add_edge("review_system_api_node", "review_system_api_aggregator_node")

    agent_builder.add_conditional_edges(
        "review_system_api_aggregator_node",
        routes.review_system_api_aggregator_router,
        ["optimize_system_api_node", END]
    )
