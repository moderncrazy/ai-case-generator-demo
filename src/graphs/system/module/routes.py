...
def optimize_system_module_tool_router(state: State) -> Literal[
    ...
    """决定调用 tool 或 生成不同角色的项目成员评审系统模块"""
    if isinstance(state["private_messages"][-1], AIMessage) and state["private_messages"][-1].tool_calls:
        logger.info(
            f"trans_id:{trans_id_ctx.get()} 子图路由:{gutils.get_func_name()} 路由至:optimize_system_module_tool_node")
        ...
        roles = [GroupMemberRole.PM,
                 ...
                 GroupMemberRole.SRE]
        logger.info(
            f"trans_id:{trans_id_ctx.get()} 子图路由:{gutils.get_func_name()} 路由至:review_system_module_node")
        return [Send("review_system_module_node", {"role": role}) for role in roles]


def review_system_module_tool_router(state: State) -> Literal[
    "review_system_module_tool_node", "review_system_module_aggregator_node"]:
    """决定是调用 tool 或 优化系统模块 或 总结系统模块问题"""
    destination_node = "review_system_module_aggregator_node"
    if isinstance(state["private_messages"][-1], AIMessage) and state["private_messages"][-1].tool_calls:
        destination_node = "review_system_module_tool_node"
