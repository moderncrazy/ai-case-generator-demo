...
from src.graphs.common import utils as cutils
...
async def review_optimization_system_module_plan_output(
    ...
    result = workflow_tool_utils.review_optimization_plan_output(
        output, "review_optimization_system_module_plan_node", runtime)
    logger.info(
        f"trans_id:{trans_id_ctx.get()} 项目Id:{project_id} 输出:{output.model_dump_json()}")
    return result
...
async def review_system_module_output(
    ...
    **功能说明：**
    ...
    Args:
        system_module_issues: list[Issue] - 针对系统模块提出的问题和建议方案列表，包含以下字段：
            - content: str - 问题描述
            - propose: str - 建议方案
        runtime: 系统运行时对象，AI传参时不用传递，会自动注入
    """
...
@tool
async def get_system_module_issues(runtime: ToolRuntime) -> str:
    """获取系统模块评审意见列表
    
    AI大模型使用此工具可获取各角色对系统模块提出的评审意见。
    
    **功能说明：**
    从运行时状态中读取并返回系统模块的评审意见，用于了解之前评审发现的问题。
    ...
    """
    project_id = runtime.state["project_id"]
    result = cutils.format_issues_to_str(runtime.state.get("system_module_issues"))
    logger.info(
...
common_tool_list = [t for t in tool_list if
                    t.name not in [
                        optimize_system_module_output.name,
                        review_system_module_output.name,
                    ]]
