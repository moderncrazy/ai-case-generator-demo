from src.graphs.system.module.schemas import SystemModule


def validate_modules_to_str(modules: list[SystemModule]) -> str:
    """校验模块列表的完整性
    ...
    error_message = ""
    if not_root:
        error_message += "没有顶级模块\n"
    if not_found:
        error_message += f"模块不存在：{",".join(not_found)}\n"
    if self_reference:
        error_message += f"模块自引用：{",".join(self_reference)}\n"
