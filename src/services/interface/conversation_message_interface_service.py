...
from src.services.business.conversation_message_service import conversation_message_service
...
class ConversationMessageInterfaceService:
    """对话消息服务
            ...
            if occupy_lock_result:
                ...
                if discuss_lock_result:
                    ...
                    async def event_generator():
                        # 创建异步任务发送心跳包 保持链接
                        queue = asyncio.Queue()
                        heart_task = asyncio.create_task(ConversationMessageInterfaceService._send_heartbeat(queue))
                        agent_task = asyncio.create_task(
                            ConversationMessageInterfaceService._start_agent(project, message, graph_file_list, queue))
                        try:
                            while True:
                                try:
                            ...
                            # 执行上下文压缩
                            background_tasks.add_task(conversation_message_service.compress_context, project.id)
